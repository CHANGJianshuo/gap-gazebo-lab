...
当前实现使用 Gazebo 真值。任务选择、状态机、重试、夹爪操作和轨迹执行由 SJM 的正式模块负责；`scripts/demo.py` 已实现仿真所需的基础阶段流程、顺序调度和异常终止，可作为独立任务管理模块的起点。未来感知和标定的团队约定见 [视觉模式草案](INTERFACES_VISION_DRAFT.md)。
