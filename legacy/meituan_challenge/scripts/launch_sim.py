...
def main():
    ...
    try:
        robot=(ROOT/'src/mtc_description/urdf/s3_latch_resolved.urdf').read_text();params=run/'robot_parameters.yaml';params.write_text(yaml.safe_dump({'robot_state_publisher':{'ros__parameters':{'robot_description':robot,'use_sim_time':True,'publish_frequency':100.}}}))
        start('robot_state_publisher',['ros2','run','robot_state_publisher','robot_state_publisher','--ros-args','--params-file',str(params)])
        cmd=['gz','sim','-s','-v','3',str(ROOT/f'src/mtc_simulation/worlds/{args.task}.sdf')]
        ...
        run_checked(['ros2','run','ros_gz_sim','create','-world','mtc','-name','mtc_s3','-file',str(ROOT/'src/mtc_description/urdf/s3_latch_resolved.urdf')],60)
